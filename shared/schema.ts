...
// Accidents Table - İş Kazaları ve Ramak Kala Olayları
export const accidents = pgTable("accidents", {
  id: varchar("id").primaryKey().default(sql`gen_random_uuid()`),
  accidentNumber: text("accident_number").notNull().unique(), // ACC-2025-001
  
  // Basic Event Information
  eventDate: timestamp("event_date").notNull(), // Olayın Gerçekleştiği Tarih
  eventTime: text("event_time").notNull(), // Olayın Gerçekleştiği Saat (HH:MM)
  eventType: text("event_type").notNull(), // "is_kazasi" | "ramak_kala"
  workShift: text("work_shift").notNull(), // "Gündüz Mesaisi" | "Gece Mesaisi"
  
  // Location Information (Cascade Dropdowns)
  eventArea: text("event_area").notNull(), // Atık Alanları, Depo Alanları, Genel - Ofis Alanları, etc.
  eventLocation: text("event_location").notNull(), // Specific location based on area
  
  // SGK and Official Information
  sgkNotificationDate: timestamp("sgk_notification_date"), // SGK'ya Bildirim Tarihi
  
  // Personnel Information
  personalIdNumber: text("personal_id_number").notNull(), // Personel Sicil No
  fullName: text("full_name").notNull(), // Ad - Soyad
  workStartDate: timestamp("work_start_date").notNull(), // İşe Başlama Tarihi
  workingDays: integer("working_days"), // Çalışma Süresi (Otomatik hesaplanacak)
  
  // Employment Information  
  employmentStatus: text("employment_status").notNull(), // Kadrolu, Kadrosuz, Stajyer, Geçici Taşeron, Sürekli Taşeron
  jobGroup: text("job_group").notNull(), // Meslek Grubu
  department: text("department").notNull(), // Departmanı
  position: text("position").notNull(), // Görevi / Unvanı
  companyAffiliation: text("company_affiliation"), // Bağlı Olduğu Firma
  
  // Accident Details
  accidentCause: text("accident_cause"), // Kazaya Sebep Olan Etmenler (Kaza Türü)
  accidentDescription: text("accident_description"), // Olay Açıklaması
  injurySeverity: text("injury_severity"), // Yaralanma Şiddeti
  bodyPart: text("body_part"), // Etkilenen Vücut Bölgesi
  
  // Investigation and Follow-up
  investigationStatus: text("investigation_status").default("pending"), // pending, in_progress, completed
  rootCause: text("root_cause"), // Kök Neden
  correctiveActions: jsonb("corrective_actions").$type<Array<{action: string, responsible: string, dueDate: string, status: string}>>().default([]),
  preventiveActions: jsonb("preventive_actions").$type<Array<{action: string, responsible: string, dueDate: string, status: string}>>().default([]),
  
  // Documentation
  images: jsonb("images").$type<string[]>().default([]), // Photo attachments
  documents: jsonb("documents").$type<string[]>().default([]), // Document attachments
  witnessNames: text("witness_names"), // Tanık İsimleri
  
  // Medical Treatment (if applicable)
  medicalTreatment: boolean("medical_treatment").default(false), // Tıbbi müdahale gerekti mi?
  hospitalName: text("hospital_name"), // Hastane adı
  treatmentDetails: text("treatment_details"), // Tedavi detayları
  workDaysLost: integer("work_days_lost").default(0), // Kayıp iş günü
  
  // Status and Metadata
  status: text("status").default("active"), // active, resolved, closed
  priority: text("priority").default("medium"), // low, medium, high, critical
  reportedBy: varchar("reported_by").references(() => users.id), // Raporu yapan kullanıcı
  assignedTo: varchar("assigned_to").references(() => users.id), // Sorumlu uzman
  locationId: varchar("location_id").references(() => locations.id), // Hastane/lokasyon
  
  createdAt: timestamp("created_at").defaultNow(),
  updatedAt: timestamp("updated_at").defaultNow(),
});

// Accident Insert Schema
export const insertAccidentSchema = createInsertSchema(accidents).omit({
  id: true,
  accidentNumber: true, // Auto-generated
  workingDays: true, // Auto-calculated
  createdAt: true,
  updatedAt: true,
});
...
// Legacy Tables (Stubs for Drizzle Kit to avoid rename detection)
...
export const pdf_template_fields = pgTable("pdf_template_fields", {
  id: varchar("id").primaryKey().default(sql`gen_random_uuid()`),
...
export const checklist_submissions = pgTable("checklist_submissions", {
  id: varchar("id").primaryKey().default(sql`gen_random_uuid()`),
  assignment_id: varchar("assignment_id"),
  inspection_id: varchar("inspection_id"),
...
export const checklist_inspections = pgTable("checklist_inspections", {
  id: varchar("id").primaryKey().default(sql`gen_random_uuid()`),
  template_id: varchar("template_id"),
  location_id: varchar("location_id"),
  inspector_id: varchar("inspector_id"),
  inspection_date: timestamp("inspection_date"),
  status: text("status"),
  tw_score: integer("tw_score"),
  total_score: integer("total_score"),
  max_possible_score: integer("max_possible_score"),
  success_percentage: integer("success_percentage"),
...
export const checklist_answers = pgTable("checklist_answers", {
  id: varchar("id").primaryKey().default(sql`gen_random_uuid()`),
  inspection_id: varchar("inspection_id"),
  ...
  tw_score: integer("tw_score"),
  calculated_score: integer("calculated_score"),
  has_photo: boolean("has_photo"),
