...
import { Users, UserPlus, Edit, Trash2, Shield, MapPin, Key, Building2, Phone, Mail, Plus, Search, X, Upload, Camera, User, Copy, Grid3X3, List, Image, CheckSquare, PlayIcon, FileBarChart, Activity, TrendingUp, Filter, Eye } from "lucide-react";
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <Select onValueChange={field.onChange} defaultValue={field.value}>
                      <FormControl>
                        <SelectTrigger data-testid="select-edit-locationid">
                          <SelectValue placeholder="Hastane seçiniz" />
                        </SelectTrigger>
                      </FormControl>
                      <SelectContent>
          ...
          <Form {...createHospitalForm}>
            <form onSubmit={createHospitalForm.handleSubmit(onCreateHospitalSubmit)} className="space-y-4">
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormLabel>Hastane Logosu (İsteğe Bağlı)</FormLabel>
                    <FormControl>
                      <div className="flex items-center gap-4">
                        {field.value && (
...
function MigrationPanel() {
  ...
  const runMigration = useMutation({
    mutationFn: async (locationIds: string[]) => {
      const response = await fetch('/api/admin/migrate-hospital-departments', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${localStorage.getItem('token')}`
        },
        body: JSON.stringify({ targetLocationIds: locationIds })
      });
      if (!response.ok) {
    ...
    onSuccess: (data) => {
      ...
      toast({
        ...
        description: `${data.successful} hastane başarıyla güncellendi`,
      });
    },
    onError: (error: any) => {
      toast({
        ...
        description: error.message || 'Migration sırasında hata oluştu',
      });
    },
  });
  ...
  const handleRunMigration = () => {
    ...
    const locationsNeedingMigration = migrationStatus.locations
    ...
    if (locationsNeedingMigration.length === 0) {
      toast({
        ...
        description: 'Tüm hastaneler güncel durumda',
      });
      return;
    }
      ...
      <Card>
        <CardHeader>
          <div className="flex items-center gap-2">
            ...
            <CardTitle>Database Migration Durumu</CardTitle>
          </div>
          <CardDescription>
            Hospital departments migration durumu ve güncelleme işlemleri
          </CardDescription>
        </CardHeader>
            ...
            <>
                ...
                <Card>
                  <CardContent className="p-4">
                    <div className="flex items-center gap-2">
                      ...
                      <div>
                        <p className="text-sm text-muted-foreground">Toplam Hastane</p>
                        <p className="text-2xl font-bold">{migrationStatus.totalLocations}</p>
                      </div>
                    </div>
                  </CardContent>
                </Card>
                <Card>
                  <CardContent className="p-4">
                    <div className="flex items-center gap-2">
                      ...
                      <div>
                        <p className="text-sm text-muted-foreground">Güncel</p>
                        <p className="text-2xl font-bold">{migrationStatus.ready}</p>
                      </div>
                    </div>
                  </CardContent>
                </Card>
                <Card>
                  <CardContent className="p-4">
                    <div className="flex items-center gap-2">
                      ...
                      <div>
                        <p className="text-sm text-muted-foreground">Migration Gerekli</p>
                        <p className="text-2xl font-bold">{migrationStatus.needsMigration}</p>
                      </div>
                    </div>
                  </CardContent>
                </Card>
              ...
              <div className="flex items-center justify-between p-4 bg-gray-50 rounded-lg">
                <div>
                  ...
                  <p className="text-sm text-muted-foreground">
                    Eksik hastanelere default bölümler eklenir ({migrationStatus.needsMigration} hastane)
                  </p>
                </div>
                <Button 
              ...
              <div className="space-y-2">
                ...
                <div className="max-h-64 overflow-y-auto space-y-2">
                  {migrationStatus.locations.map((location: any) => (
                    <div key={location.id} className="flex items-center justify-between p-3 border rounded">
                      ...
                      <div className="flex items-center gap-2">
                        <Badge variant={location.needsMigration ? "destructive" : "secondary"}>
                          {location.departmentCount} bölüm
                        </Badge>
              ...
              {migrationResults && (
                <Card>
                  <CardHeader>
                    <CardTitle className="text-green-600">Son Migration Sonucu</CardTitle>
                  </CardHeader>
                  <CardContent className="space-y-2">
                    <p><strong>Başarılı:</strong> {migrationResults.successful}</p>
                    <p><strong>Atlanan:</strong> {migrationResults.skipped}</p>
                    <p><strong>Hatalı:</strong> {migrationResults.errors}</p>
                    {migrationResults.results.length > 0 && (
                      <details className="mt-4">
                        <summary className="cursor-pointer font-medium">Detaylı Sonuçlar</summary>
